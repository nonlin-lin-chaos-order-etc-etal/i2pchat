...
FileTransferManager::FileTransferManager(AppContext& appCtx)
:mCore(appCtx)
{
...
CFileTransferRecive * FileTransferManager::getFileTransferReciveByID(qint32 ID)const
{
...
CFileTransferSend* FileTransferManager::getFileTransferSendsByID(qint32 ID)const
{
...
const QList< CFileTransferSend * > FileTransferManager::getFileTransferSendsList()const
{
	return mFileSends;
}

const QList< CFileTransferRecive * > FileTransferManager::getFileTransferReciveList()const
{
	return mFileRecives;
}

void FileTransferManager::addNewFileTransfer(QString FilePath, QString Destination)
{

	QString Protocolversion;
	double  ProtoVersionD=0.0;

	AbstractUserLocalImage* AbstractUserLocalImage=mCore.getUserManager()->getUserByI2P_Destination(Destination);
	if(AbstractUserLocalImage!=NULL){
		Protocolversion=AbstractUserLocalImage->getHighestUsableProtocolVersionFiletransfer();
		ProtoVersionD  =AbstractUserLocalImage->getHighestUsableProtocolVersionFiletransfer_D();
	}
...
void FileTransferManager::addNewFileRecive(qint32 ID, QString FileName, QString FileSize,QString Destination,QString ProtocolVersion)
{	
	...
	if(OK==false){
		qWarning()<<"File\t"<<__FILE__<<endl
			  <<"Line:\t"<<__LINE__<<endl
			  <<"Function:\t"<<" FileTransferManager::addNewFileRecive"<<endl
	...
	if(ProtocolVersionD > FileTransferProtocol::MAXPROTOCOLVERSION_D){
		...
		AbstractUserLocalImage* AbstractUserLocalImage=mCore.getUserManager()->getUserByI2P_Destination(Destination);
		if(AbstractUserLocalImage!=NULL){
			AbstractUserLocalImage->slotIncomingMessageFromSystem(tr("Ignore Incoming Filetransfer, " \
...
bool FileTransferManager::isThisID_a_FileSendID(qint32 ID) const
{
	for(int i=0;i<mFileSends.size();i++){
		if(mFileSends.at(i)->getStreamID()==ID){
			return true;
...
bool FileTransferManager::isThisID_a_FileReciveID(qint32 ID) const
{
	for(int i=0;i<mFileRecives.size();i++){
		if(mFileRecives.at(i)->getStreamID()==ID){
			return true;
...
bool FileTransferManager::checkIfAFileTransferOrReciveisActive() const
{
...
void FileTransferManager::removeFileTransfer(const qint32 ID)
{
...
void FileTransferManager::removeFileRecive(const qint32 ID)
{	
...
const QString FileTransferManager::FilterForFileName(QString FileName)const 
{
    FileName.replace("\\","");
    FileName.replace("/","");
    FileName.replace("..","");
