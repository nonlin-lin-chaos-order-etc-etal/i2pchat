...
UserManager::UserManager(AppContext& appCtx, QString UserFileWithPath,UnsentChatMessagesStorage& UnsentChatMessageStorage)
:
  mCore(appCtx),mUserFileWithPath(UserFileWithPath),mUnsentMessageStorage(UnsentChatMessageStorage),rosterModel(nullptr)
{
...
AbstractUserLocalImage* UserManager::getUserByI2P_ID(qint32 ID)const{
...
AbstractUserLocalImage* UserManager::getUserByI2P_Destination(QString Destination)const{
	for(int i=0;i<mUsers.size();i++){
...
QString UserManager::getUserInfosByI2P_Destination(QString Destination)const
{
	QString Infos="No Informations recived";

	for(int i=0;i<mUsers.size();i++){
		if(mUsers.at(i)->getI2PDestination()==Destination){
			AbstractUserLocalImage* theUser=mUsers.at(i);
			...
			if(theUser->getProtocolVersion_D()>=0.3){
				UserInfo recivedInfos=theUser->getRecivedUserInfos();
				QString sAge;
...
const QList<AbstractUserLocalImage*> UserManager::getUserList()const{
	return mUsers;
}

bool UserManager::validateI2PDestination(const QString I2PDestination) const
{
...
bool UserManager::addNewUser(QString Name,QString I2PDestination,qint32 I2PStream_ID,bool SaveUserList,
                              AbstractUserLocalImage**resultingNewUser, bool /*isSwarmUser*/){
    if(resultingNewUser){
        (*resultingNewUser)=nullptr;
    }
	UserBlockManager& UserBlockManager=*(mCore.getUserBlockManager());
	Connector& Protocol=*(mCore.getProtocol());
	...
	if(isValid==false){
		qCritical()<<"File\t"<<__FILE__<<endl
			   <<"Line:\t"<<__LINE__<<endl
			   <<"Function:\t"<<"UserManager::addNewUser"<<endl
			   <<"Message:\t"<<"Destination is not valid"<<endl
			   <<"Destination:\t"<<I2PDestination<<endl
			   <<"action add new AbstractUserLocalImage ignored"<<endl;
	...
	//add newuser
	SoundManager& SoundManager=*(mCore.getSoundManager());
	AbstractUserLocalImage* newuser=new AbstractUserLocalImage(mCore, Protocol,Name,I2PDestination,I2PStream_ID);
...
bool UserManager::checkIfUserExitsByI2PDestination(const QString I2PDestination)const{
...
void UserManager::changeUserPositionInUserList(int oldPos, int newPos)
{
...
bool UserManager::deleteUserByI2PDestination(QString I2PDestination){
	for(int i=0;i<mUsers.count();i++){
		if(mUsers.at(i)->getI2PDestination()==I2PDestination){
...
bool UserManager::renameUserByI2PDestination(const QString Destination, const QString newNickname){
	for(int i=0;i<mUsers.size();i++){
		if(mUsers.at(i)->getI2PDestination()==Destination){
...
void UserManager::avatarImageChanged()
{
	for(int i=0;i<mUsers.count();i++){
	  AbstractUserLocalImage* AbstractUserLocalImage=mUsers.at(i);
	  
	  if(AbstractUserLocalImage->getOnlineStatus()!= USEROFFLINE &&  
	     AbstractUserLocalImage->getOnlineStatus()!= USERTRYTOCONNECT &&
	     AbstractUserLocalImage->getOnlineStatus()!= USERBLOCKEDYOU &&
	     AbstractUserLocalImage->getProtocolVersion_D()>=0.6){
		Connector& Protocol=*(mCore.getProtocol());
...
void UserManager::slotSaveUnsentMessageForDest(QString I2PDest)
{
  AbstractUserLocalImage* theUser=getUserByI2P_Destination(I2PDest);
  if(theUser!=NULL){	
    ...
    qWarning()  <<"File\t"<<__FILE__<<endl
	        <<"Line:\t"<<__LINE__<<endl
	        <<"Function:\t"<<"UserManager::slotSaveUnsentMessageForDest"<<endl
		<<"Message:\t"<<"No AbstractUserLocalImage found with this dest"<<endl
		<<"I2PDest.:\t"<<I2PDest<<endl;
  }
}

void UserManager::setRosterModel(Roster* rosterModel_) {
