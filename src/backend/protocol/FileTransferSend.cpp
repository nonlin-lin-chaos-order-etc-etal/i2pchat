...
CFileTransferSend::CFileTransferSend(AppContext& appCtx,ConnectionManager& ConnectionManager, 
		QString FilePath,QString Destination,QString Protocolversion,double ProtocolversionD)
		
	:mCore(appCtx),mConnectionManager(ConnectionManager),mFilePath(FilePath),mDestination(Destination),
		...
		case (SAM_Message_Types::CLOSED):{
			...
			emit signFileTransferFinishedOK();
				...
				if(mAllreadySendedSize==0){
					mCore.getUserManager()->getUserByI2P_Destination(mDestination)->
					slotIncomingMessageFromSystem(tr("Can't connect the AbstractUserLocalImage for Filetransfer<br>Filename: %1").arg(mFileName));
				}
				else
				{
					mCore.getUserManager()->getUserByI2P_Destination(mDestination)->
