...
CFileTransferRecive::CFileTransferRecive(AppContext& appCtx,CI2PStream& Stream, qint32 StreamID, 
			QString FileName, quint64 FileSize,QString Destination,QString Protocolversion,double ProtocolversionD)

:mCore(appCtx),mStream(Stream),mStreamID(StreamID),mFileSize(FileSize),mDestination(Destination),
  mUsingProtocolVersion(Protocolversion),mUsingProtocolVersionD(ProtocolversionD)
{
	...
	settings.beginGroup("General");
		...
		if(settings.value("UseIncomingSubFolderForEveryUser",false).toBool()==true){
		  AbstractUserLocalImage* theUser=mCore.getUserManager()->getUserByI2P_Destination(mStream.getDestination());
		  if(theUser!=NULL){
