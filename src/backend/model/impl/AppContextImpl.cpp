...
AppContext::AppContext(QString configPath) : rosterController(nullptr)
{	
    mConfigPath=configPath;

    mDebugMessageHandler= new DebugMessageManager("General", configPath);
    mSoundManager= new SoundManager(mConfigPath);
    ...
    mProtocol= new Connector(*this);
    this->mCurrentOnlineStatus=AbstractUserLocalImage::USEROFFLINE;
...
/*
    ...
    if((settings.value("Enabled",false).toBool()) ==true && getUserInfos().Nickname.isEmpty()==false){
        ...
        mDebugSeedlessHandler= new DebugMessageManager("Usersearch");
...
*/
    mUnsentChatMessageStorage= new UnsentChatMessagesStorage(mConfigPath+"/UnsentChatMessageStorage.ini");
    mUserBlockManager= new UserBlockManager(*this,mConfigPath+"/UserBlockList.dat");
    mUserManager= new UserManager(*this,mConfigPath+"/users.config",*mUnsentChatMessageStorage);
    ...
    mFileTransferManager= new FileTransferManager(*this);
    connect(mFileTransferManager,SIGNAL(signUserStatusChanged()),this,
            SIGNAL(signUserStatusChanged()));

    Roster * rosterModel = new Roster(*this, *mUserManager, *mUnsentChatMessageStorage);
    rosterController = new RosterController(*rosterModel);
    mUserManager->setRosterModel(rosterModel);

    swarmsManager = new SwarmsController();
}
...
AppContext::~AppContext(){
    ...
    Roster * rosterModel = & (rosterController->getRosterModel());
    delete rosterController;
    delete rosterModel;
}

void AppContext::doNamingLookUP(QString Name)const{
    mConnectionManager->doNamingLookUP(Name);
}

QString AppContext::calcSessionOptionString() const
{
...
void AppContext::slotStreamStatusRecived(const SAM_Message_Types::RESULT result,const qint32 ID,QString Message) {

    CI2PStream* stream=mConnectionManager->getStreamObjectByID(ID);
    AbstractUserLocalImage* user=NULL;
    QString Destination;
    ...
    if(result==SAM_Message_Types::OK && ID>0){
        ...
        if(user==NULL){
            ...
            if(user->getOnlineStatus()!=USEROFFLINE && user->getOnlineStatus()!=USERTRYTOCONNECT){
        ...
        if(user->getUsedB32Dest()==true){
            ...
            mConnectionManager->doDestroyStreamObjectByID(ID);
            user->slotIncomingMessageFromSystem(tr("Invalid AbstractUserLocalImage - Destination: please delete the user\n"));
...
void AppContext::setUserProtocolVersionByStreamID(qint32 ID,QString Version){
    AbstractUserLocalImage* theUser=mUserManager->getUserByI2P_ID(ID);
...
void AppContext::closeAllActiveConnections(){
    //close all known Online||TrytoConnect Connections
    QList<AbstractUserLocalImage*> users=mUserManager->getUserList();
    ...
    for(int i=0;i<users.size();i++){
                ...
                users.at(i)->getConnectionStatus()==TRYTOCONNECT)
        {
            ...
            users.at(i)->setConnectionStatus(AbstractUserLocalImage::OFFLINE);
            users.at(i)->setOnlineState(USEROFFLINE);
        }
    }
}

void AppContext::slotNamingReplyRecived(const SAM_Message_Types::RESULT result,QString Name,QString Value,QString Message){
    if(result==SAM_Message_Types::OK && Name=="ME"&& mMyDestination.isEmpty()){
        ...
        AbstractUserLocalImage* theUser=mUserManager->getUserByI2P_Destination(Name);
...
const QString AppContext::getMyDestination()const{
    return this->mMyDestination;
}

void AppContext::deletePacketManagerByID(qint32 ID){
...
QString AppContext::getConnectionDump()const{
    ...
    for(int n=0;n<allStreamsList.size();n++){
        ...
        QString StreamID;
        AbstractUserLocalImage* theUser=NULL;
...
ONLINE_STATE AppContext::getOnlineStatus() const
{
    return this->mCurrentOnlineStatus;
}

void AppContext::setOnlineStatus(const ONLINE_STATE newStatus)
{
    ...
    if(newStatus==AbstractUserLocalImage::USEROFFLINE){
    ...
    {
        //send new Status to every connected AbstractUserLocalImage
        this->mCurrentOnlineStatus=newStatus;
        QList<AbstractUserLocalImage*> Users=mUserManager->getUserList();
...
void AppContext::restartCore()
{
    this->init();
}

QString AppContext::getDestinationByID(qint32 ID) const
{
    AbstractUserLocalImage* user=mUserManager->getUserByI2P_ID(ID);
    if(user!=NULL){
...
void AppContext::slotStreamControllerStatusOK(bool Status)
{
    if(Status==true){
...
void AppContext::createStreamObjectsForAllUsers()
{
    QList<AbstractUserLocalImage*> users=mUserManager->getUserList();
...
void AppContext::setStreamTypeToKnown(qint32 ID,const QByteArray Data,bool isFileTransfer_Recive)
{
...
CI2PStream * AppContext::getI2PStreamObjectByID(qint32 ID)const
{
    return mConnectionManager->getStreamObjectByID(ID);
}

void AppContext::slotIncomingStream(CI2PStream * stream)
{
...
void AppContext::createStreamObjectForUser(AbstractUserLocalImage& AbstractUserLocalImage)
{
    qint32 msec=0;
    qint32 ID=AbstractUserLocalImage.getI2PStreamID();
    ...
    AbstractUserLocalImage.setI2PStreamID(t->getID());

    if(t!=NULL){
        connect(t,SIGNAL(signDataRecived(const qint32, const QByteArray)),mProtocol,
                SLOT(slotInputUnknown(const qint32, const QByteArray)));
        t->doConnect(AbstractUserLocalImage.getI2PDestination());
...
void AppContext::slotNewSamPrivKeyGenerated(const QString SamPrivKey)
{
    QSettings* settings= new QSettings(mConfigPath+"/application.ini",QSettings::IniFormat);
    settings->beginGroup("Network");
...
bool AppContext::useThisChatConnection(const QString Destination, const qint32 ID)
{
    AbstractUserLocalImage* theUser=NULL;
...
const UserInfo AppContext::getUserInfos() const
{
    return mUserInfos;
}
...
void AppContext::doConvertNumberToTransferSize(quint64 inNumber, QString & outNumber, QString& outType,bool addStoOutType)const
{
    QString SSize;
...
const QString AppContext::getMyDestinationB32() const
{
    return mMyDestinationB32;
}


void AppContext::setMyDestinationB32(QString B32Dest)
{
...
QString AppContext::canonicalizeTopicId(QString topicIdNonCanonicalized) {

    //FIXME canonicalizeTopicId(topicIdNonCanonicalized);
    return topicIdNonCanonicalized;
}

Roster& AppContext::getRosterModel() { return rosterController->getRosterModel(); }
RosterController& AppContext::getRosterController() { return *rosterController; }
